...
class ExpertBackend(nn.Module):
    def __init__(self, name: str, expert: nn.Module, opt: torch.optim.Optimizer, *,
        ...
        assert len(args_schema) or len(kwargs_schema), "expert must receive at least one positional or keyword input." \
        ...
        self.forward_pool = TaskPool(
            self.forward, inputs_schema=tuple(nested_flatten(self.forward_schema)),
            outputs_schema=tuple(nested_flatten(self.outputs_schema)), uid=f'{self.name}_forward', **kwargs)
        ...
        self.backward_pool = TaskPool(
            self.backward, inputs_schema=tuple(nested_flatten(self.backward_schema)),
            outputs_schema=tuple(nested_flatten(self.forward_schema)),  # return grads w.r.t. inputs with same schema
            uid=f'{self.name}_backward', **kwargs)
