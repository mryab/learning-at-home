...
class EmulatedFaultyDMoE(nn.Module):
    def __init__(self, in_features, num_experts, num_active, update_every_inputs, update_every_steps, failure_rate,
                 Expert, Optimizer):
        """
    ...
    def forward(self, input):
        ...
        if self.failure_rate != 0:
            gating_logits = torch.where(torch.rand_like(gating_logits) < self.failure_rate,
                                        gating_logits - float('inf'), gating_logits)
...
def get_non_expert_params(model):
    expert_params = set(param for module in model.modules() if isinstance(module, EmulatedFaultyDMoE)
